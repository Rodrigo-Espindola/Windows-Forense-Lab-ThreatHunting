...
- [O Papel do Docker (Elasticsearch + Kibana)](#o-papel-do-docker-elasticsearch--kibana)
- [A Simulação de Cenários Maliciosos (Scripts no Host)](#a-simulação-de-cenários-maliciosos-scripts-no-host)
- [O Papel do Sysmon](#o-papel-do-sysmon)
- [A Lógica de Coleta e Envio: Elastic Agent Standalone](#a-lógica-de-coleta-e-envio-elastic-agent-standalone)
- [Objetivo Profissional e Conclusão](#objetivo-profissional-e-conclusão)
...
> ⚠️ **Nota de segurança:** neste arquivo, `xpack.security.enabled=false` está desativado propositalmente, pois este é um ambiente de estudo local, isolado e acessado apenas via `localhost`. Essa configuração **nunca** deveria ser replicada em um ambiente de produção — lá, autenticação, TLS/SSL e controle de acesso (RBAC) seriam obrigatórios. A decisão aqui é consciente e serve para simplificar o setup de um laboratório pessoal, não reflete desconhecimento sobre hardening de produção.
...
> ⚠️ **Nota de segurança:** assim como no Elasticsearch, `ssl.verification_mode: none` é aceitável apenas porque o tráfego ocorre inteiramente em `localhost`, dentro de um ambiente isolado. Em produção, a verificação de certificado SSL/TLS seria obrigatória entre o agente e o output.
